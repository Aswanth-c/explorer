...
import SvgThreeDotsHorizontal from "../../../../app/[network]/[type]/(standard)/[query]/[[...viewPath]]/(components)/(icons)/ThreeDotsHorizontal";
...
export function TableEntry({ entity }: Props) {
  ...
  const openTransaction = () => {
    console.log(entity.components.row.data.link);
    router.push(`transaction/${entity.components.row.data.link?.query}`);
  };
  return (
        ...
        <Popover.Root>
          ...
          <Popover.Portal>
            <Popover.Content className="PopoverContent" sideOffset={5}>
              <div
                className=" bg-white rounded-xl w-44 border-solid border-[1px] border-[#2A2B2E1A] absolute right-0 shadow-md"
                onClick={() => {
                  openTransaction();
                }}
              >
